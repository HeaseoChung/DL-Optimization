...
class EngineInferencer(object):
    def __init__(self, trt_engine_path, height=-1, width=-1, channel=3,scale=2, dynamic=False):
        ...
        if not dynamic:
            ...
            self.inputs, self.outputs, self.bindings = self.dynamicBindingProcess(height, width,scale, channel)
    ...
    def dynamicBindingProcess(self, height, width,scale, channel=3):
        ...
        for i, binding in enumerate(self.engine):
            if self.engine.binding_is_input(binding):
                ...
                self.context.set_binding_shape(i, (1, channel, height*scale, width*scale))
